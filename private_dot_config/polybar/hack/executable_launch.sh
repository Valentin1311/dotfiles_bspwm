...
# Launch bar 
polybar -c ~/.config/polybar/hack/config.ini top & 

if [[ ! -z ${external_monitor} ]]; then
  last_monitor=$(cat ~/.config/polybar/last_monitor.txt | head -n 1)
  ## Replace last_monitor by current in polybar config
  sed -i "/eDP/! s/${last_monitor}/${external_monitor}/g" ~/.config/polybar/hack/config.ini
  ## Update last_monitor
  echo $external_monitor > ~/.config/polybar/last_monitor.txt

  polybar -c ~/.config/polybar/hack/config.ini external &
fi